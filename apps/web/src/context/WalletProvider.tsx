...
import { rainbowConfig, web3AuthInstance } from "@beratrax/web/src/config/walletConfig";
...
import { defaultChainId } from "@beratrax/core/src/config/constants";
...
const WalletProvider: React.FC<IProps> = ({ children }) => {
	...
	const { address: rainbowkitAddress, status, isConnecting: isConnectingWagmi, isReconnecting, connector } = useAccount();
	...
	const displayAccount = React.useMemo(
		() => (currentWallet ? `${currentWallet.substring(0, 6)}...${currentWallet.substring(currentWallet.length - 5)}` : ""),
		[currentWallet]
	);
	const getPkey = async () => {
		try {
