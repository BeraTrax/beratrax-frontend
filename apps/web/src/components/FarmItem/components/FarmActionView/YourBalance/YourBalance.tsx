		...
		<div className="flex-1">
			<div className="flex items-center gap-x-3">
				<div className="flex flex-col">
					<h1 className="text-green-500 text-lg font-medium flex items-center gap-x-2">${customCommify(earningsValueUsd.toFixed(2))}</h1>
				</div>
...
const YourBalance = ({ farm }: { farm: PoolDef }) => {
	...
	const renderEarningsSection = () => {
		return (
			<div className="w-full md:w-1/2 flex flex-col">
				<h3 className="text-textWhite font-arame-mono font-normal text-[16px] leading-[18px] tracking-widest">YOUR EARNINGS</h3>
					...
					{isVaultEarningsFirstLoad ? (
						...
						<>
							<div className="flex flex-row gap-4">
								<TokenEarning earnings={farmEarnings.earnings0} token={farmEarnings.token0} chainId={farm.chainId} prices={prices} />
								{farmEarnings?.earnings1 && (
									<TokenEarning earnings={farmEarnings.earnings1} token={farmEarnings.token1} chainId={farm.chainId} prices={prices} />
								)}
							</div>
						</>
					)}
	...
	const renderPositionSection = () => {
		return (
			<div className="w-full md:w-1/2 flex flex-col">
				<h3 className="text-textWhite font-arame-mono font-normal text-[16px] leading-[18px] tracking-widest">YOUR POSITION</h3>
				<div className="bg-bgDark py-4 px-4 mt-2 rounded-2xl backdrop-blur-lg flex-1 flex flex-col justify-center">
					{isLoading || isConnecting ? (
						...
						<>
							<div className="flex items-center gap-x-3">
								<h1 className="text-textWhite text-lg font-medium">${stakedTokenValueUsd ? formatCurrency(stakedTokenValueUsd) : 0}</h1>
							</div>
							<div className="flex items-center gap-x-2 mt-2">
