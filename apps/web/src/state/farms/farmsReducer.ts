...
import { CHAIN_ID, FarmOriginPlatform, FarmTransactionType } from "src/types/enums";
import { sleep, toEth } from "src/utils/common";
import { Address, erc20Abi, getContract, getAddress } from "viem";
    ...
    async (
        ...
        try {
            ...
            for (let i = 0; i < vaultBalancesResponse.length; i += 2) {
                ...
                let expectedLpAmount =
                    balance * BigInt(balances[farms[i / 2].chainId][farms[i / 2].vault_addr].valueWei);

                if (totalSupplies[farms[i / 2].chainId][farms[i / 2].vault_addr].supplyWei !== "0")
                    expectedLpAmount =
                        expectedLpAmount /
                        BigInt(totalSupplies[farms[i / 2].chainId][farms[i / 2].vault_addr].supplyWei);
