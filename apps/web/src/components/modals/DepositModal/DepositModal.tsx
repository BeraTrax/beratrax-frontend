...
const DepositModal: FC<IProps> = ({ handleClose, handleSubmit, farmId, inputAmount, symbol, max, token }) => {
	...
	useEffect(() => {
		(async function () {
			...
			const toBal = await getBalance(symbol === "usdc" ? addressesByChainId[farm.chainId].nativeUsdAddress! : zeroAddress, currentWallet, {
					...
					{Object.entries(combined.chainBalances)
						...
						.map(([key, value]) => (
							<div key={key} style={{ display: "flex", gap: 20 }}>
								...
								<p>
									{(
										Number(toEth(value, symbol === "usdc" ? 6 : 18)) * (symbol === "usdc" ? 1 : prices[key as any][zeroAddress])
									).toLocaleString()}{" "}
									$
								</p>
							</div>
