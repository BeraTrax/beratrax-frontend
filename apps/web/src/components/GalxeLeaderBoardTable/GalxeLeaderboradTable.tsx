...
import bbtxLogo from "../../assets/images/btxLogo.png";
      ...
      <div className="text-textWhite flex flex-col gap-2">
        {isLoading ? (
          ...
          allEntries.map((userTVL) => {
            ...
            return (
              <StatsTableRow key={userTVL.id} index={userTVL.galxeRanking} isCurrentUser={isCurrentUser} {...userTVL} />
            );
          })
        ) : (
          <EmptyTable />
...
const StatsTableRow = ({ isCurrentUser, ...props }: { isCurrentUser?: boolean } & Record<any, any>) => {
  ...
  const { address, tvl, accountInfo, index, galxeReferralCount, earnedTraxPointsGalxe, earnedTraxByGalxeReferral } =
    props;
    ...
    <div className="bg-gradient-to-r from-buttonPrimary to-gradientSecondary p-[1px] rounded-2xl mb-4">
      <div className="bg-bgDark rounded-2xl px-4 py-6">
        <div className="flex justify-between items-center">
          ...
          <p>
            {Number(userPosition.earnedTraxPointsGalxe + userPosition.earnedTraxByGalxeReferral).toLocaleString(
              "en-us"
            )}
          </p>
        </div>
