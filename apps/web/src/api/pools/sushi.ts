...
let sushi: DynamicFarmFunctions = function (farmId) {
    ...
    const getProcessedFarmData: GetFarmDataProcessedFn = (balances, prices, decimals, vaultTotalSupply) => {
        ...
        const vaultBalance = BigInt(balances[farm.chainId][farm.vault_addr].valueWei || 0);
        ...
        const ethCurrentChainBalance = Number(toEth(combinedEthBalance.chainBalances[farm.chainId], 18));
        ...
        let depositableAmounts: TokenAmounts[] = [
            {
                ...
                tokenSymbol: "USDC",
                amount: usdcCurrentChainBalance.toString(),
                amountDollar: usdcCurrentChainBalance.toString(),
                price: prices[farm.chainId][usdcAddress],
            },
            {
                tokenAddress: zeroAddress,
                tokenSymbol: combinedEthBalance.symbol,
                amount: ethCurrentChainBalance.toString(),
                amountDollar: (Number(ethCurrentChainBalance) * ethPrice).toString(),
                price: ethPrice,
            },
            {
                tokenAddress: lpAddress,
                tokenSymbol: farm.name,
                amount: toEth(BigInt(balances[farm.chainId][lpAddress].valueWei!), decimals[farm.chainId][lpAddress]),
                amountDollar: (
                    Number(toEth(BigInt(balances[farm.chainId][lpAddress].valueWei!), decimals[farm.chainId][lpAddress])) *
