...
import useTransfer from "@beratrax/core/src/hooks/useTransfer";
import { CHAIN_ID } from "@beratrax/core/src/types/enums";
import { DEAD_ADDRESS, TRAX_TOKEN_ADDRESS } from "@beratrax/core/src/config/constants";
import { AirdropHookConfig, AirdropHookReturn, TokenActionType } from "@beratrax/core/src/types/airdrop";

const useAirdrop = (config: AirdropHookConfig): AirdropHookReturn => {
	const dispatch = useAppDispatch();
	const { reloadBalances } = useTokens();
	const { getClients, currentWallet, isSocial } = useWallet();
	...
	const handleClaim = async () => {
		...
		try {
			...
			if (config.type === "regular") {
				...
				await dispatch(claimAdditionalAirdrop({ claim: true, nonce: claimData.nonce || 0, getClients })).unwrap();
			}
	...
	const handleStake = async () => {
		...
		try {
			...
			if (config.type === "regular") {
				...
				await dispatch(claimAdditionalAirdrop({ claim: false, nonce: claimData.nonce || 0, getClients })).unwrap();
			}
