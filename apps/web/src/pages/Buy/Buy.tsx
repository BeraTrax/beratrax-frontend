...
import { defaultChainId, defaultNetworkName, RAMP_TRANSAK_API_KEY, HOLYHELD_API_KEY } from "@beratrax/core/src/config/constants";
import transaklogo from "@beratrax/core/src/assets/images/transaklogo.png";
import holyheldlogo from "@beratrax/core/src/assets/images/holyheldlogo.png";
...
export const Buy: React.FC = () => {
	...
	const holyheldSDK = useMemo(() => new HolyheldSDK({ apiKey: HOLYHELD_API_KEY }), []);
	...
	const usdToEurToUsd = useCallback(
		(amount: number, isUSD = true) => {
			...
			const convertedAmount = isUSD ? amount * usdToEurRate : amount * eurToUsdRate;
	...
	const handleHolyheldBuy = async () => {
		...
		try {
			...
			const result = await holyheldSDK.evm.onRamp.watchRequestId(onrampRequest.requestUid, {
	...
	const tabs = [
		...
		{
			id: BuyService.transak,
			label: BuyService.transak[0].toUpperCase() + BuyService.transak.slice(1),
			icon: <img src={transaklogo} alt={BuyService.transak} className="w-5 h-5 rounded-full" />,
		},
		{
			id: BuyService.holyheld,
			label: BuyService.holyheld[0].toUpperCase() + BuyService.holyheld.slice(1),
			icon: <img src={holyheldlogo} alt={BuyService.holyheld} className="w-5 h-5 rounded-full" />,
		},
	];
						...
						{tabs.map((tab) => (
							<button
								...
								className={`flex-1 flex items-center justify-center gap-2 py-3 px-4 rounded-md transition-all font-medium ${
									selectedService === tab.id ? "bg-buttonPrimary text-bgDark" : "text-textWhite hover:bg-bgDark"
					...
					<div className="mb-6  h-[73vh]">
						<div className="bg-bgDark rounded-2xl border border-gradientSecondary p-4 h-[inherit] overflow-scroll">
							<div className="flex items-center justify-between mb-4">
								...
								<button onClick={handleCloseTransakWidget} className="text-textSecondary hover:text-textWhite transition-colors">
									✕
								</button>
							</div>
					...
					<div className="bg-bgDark rounded-2xl border border-gradientSecondary p-6 mb-4">
						...
						<div className="mb-4">
							{selectedService === BuyService.coinbase ? (
								...
								<button
									...
									disabled={!isValidAmount() || isHolyheldLoading || (selectedService === BuyService.holyheld && !isHolyheldAllowed)}
									className={`w-full py-4 rounded-lg font-semibold text-lg transition-all flex items-center justify-center gap-2 ${
										isValidAmount() && !isHolyheldLoading && (selectedService !== BuyService.holyheld || isHolyheldAllowed)
											? "bg-buttonPrimary hover:bg-buttonPrimaryLight text-bgDark"
						...
						<div className="text-center">
							<p className="text-xs text-textSecondary">
								{Object.values(BuyService).map(
									(service) =>
										service === selectedService && <span key={service}>{`Powered by ${service[0].toUpperCase() + service.slice(1)}`}</span>
								)}
							</p>
							{selectedService === BuyService.holyheld && isHolyheldLoading && (
								<p className="text-xs text-buttonPrimary mt-1">Please confirm in your Holyheld mobile app</p>
							)}
						</div>
					</div>
				)}
