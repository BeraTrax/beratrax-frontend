...
import { formatDate } from "@beratrax/core/src/utils/common";
					...
					{runData.length === 0 ? (
						...
						runData.map((data, index) => {
							...
							const isBlockchainError =
								boxData.tooltip.toLowerCase().includes("reverted") || boxData.tooltip.toLowerCase().includes("blockchain error");
							...
							return (
								...
								>
									...
									<div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 hidden group-hover:block bg-bgDark text-white text-xs p-2 rounded-md z-10 min-w-[200px] shadow-lg border border-borderDark">
										...
										{isBlockchainError && <div className="text-orange-400 mb-1 font-semibold">⚠️ Blockchain Error Detected</div>}
										...
										<div
											className={`mt-1 ${
												data.status === "success" ? "text-green-400" : data.status === "error" ? "text-red-600" : "text-textSecondary"
											}`}
										>
											Status: {data.status}
										</div>
