...
export const updatePrices = createAsyncThunk("prices/updatePrices", async (_, thunkApi) => {
	try {
		...
		if (data && data?.[CHAIN_ID.BERACHAIN] && data?.[CHAIN_ID.BERACHAIN]?.["0x6969696969696969696969696969696969696969"]) {
			// wBERA
					...
					await Promise.all(
						stablePools.map(async (pool) => {
							...
							totalSupplies[Number(chainId)][pool.lp_address] = {
								...
								supplyUsdFormatted: formatCurrency(Number(formatUnits(actualTotalSupply, 18)) * prices[Number(chainId)][pool.lp_address]),
							};
						})
					);
				})
