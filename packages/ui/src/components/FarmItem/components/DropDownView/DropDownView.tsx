		...
		<View className={`dropdown_menu ${lightMode && "dropdown_menu--light"}`}>
			<View className="basic_container">
				<View className="type_tab">
					<Tabs>
						<PoolButton
							onPress={() => !IS_LEGACY && setFarmOptions({ transactionType: FarmTransactionType.Deposit })}
							description={FarmTransactionType.Deposit}
							active={transactionType === FarmTransactionType.Deposit}
						/>
						<PoolButton
							onPress={() => !IS_LEGACY && setFarmOptions({ transactionType: FarmTransactionType.Withdraw })}
			...
			{!showMoreDetail ? (
				<Pressable className={`see_details_dropdown ${lightMode && "see_details_dropdown--light"}`} onPress={() => setShowMoreDetail(true)}>
					<Text className={`see_details_description ${lightMode && "see_details_description--light"}`}>See more details</Text>
					<RiArrowDownSLine />
				</Pressable>
			) : (
