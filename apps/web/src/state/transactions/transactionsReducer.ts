    ...
    async (
        ...
        try {
            ...
            return _thunkApi.rejectWithValue(
                error instanceof Error ? error.message : "Failed to edit transaction step"
            );
        }
    }
);
    ...
    async ({ walletAddress }: { walletAddress: Address }, _thunkApi) => {
        try {
            ...
            const res = await backendApi.get(
                `transaction/tx-history?from=${walletAddress}&limit=${limit}&sort=-date${
                    tx ? `&_id[lt]=${tx._id}` : ""
                }`
            );
            ...
            await Promise.all(
                txs
                    ...
                    .map(async (item) => {
                        ...
                        const { beforeBridgeBalance, bridgeService, fromChain, toChain, txHash } =
                            bridgeStep.bridgeInfo;
