				...
				<div className="flex justify-between items-center">
					...
					<div className="flex items-center gap-2">
						<div className="flex justify-center items-center gap-y-2 mr-2">
							{farm?.logo1 && <img alt={farm?.alt1} className="mr-[-5px] h-8 md:mr-0 lg:h-14 max-w-fit rounded-full" src={farm?.logo1} />}
							{farm?.logo2 && (
								<img
									alt={farm?.alt2}
						...
						<div>
							{farmApys && toFixedFloor((farm.isUpcoming ? farm.total_apy : farmApys?.apy + farmApys?.pointsApr) || 0, 2) === 0 ? (
								...
								<div className="flex flex-col justify-end items-end">
									...
									<Tooltip anchorId={key} className="shadow-md rounded-md bg-bgSecondary text-white px-2 py-1 leading-none" />
								</div>
							)}
						</div>
					</div>
								...
								{farmApys && toFixedFloor((farm.isUpcoming ? farm.total_apy : farmApys?.apy + farmApys?.pointsApr) || 0, 2) === 0 ? (
									...
									<>
										<p className="text-textWhite text-right">
											...
											<Tooltip anchorId={key2} className="shadow-md rounded-md bg-bgDark text-white px-2 py-1 leading-none" />
										</p>
									</>
								)}
							</>
			...
			<div className="flex justify-between gap-5 px-6 py-4 text-center">
				...
				<div className="flex justify-center flex-row items-center">
					<div className="flex mr-2">
						...
						{farm.logo2 ? <img alt={farm.alt2} className={`w-11 h-11 md:w-14 md:h-14 ml-[-10px] rounded-full`} src={farm.logo2} /> : null}
						{farm.logo3 ? <img alt={farm.alt3} className={`w-11 h-11 md:w-14 md:h-14 ml-[-10px] rounded-full`} src={farm.logo3} /> : null}
					</div>
				...
				<div className={`flex flex-col min-w-14 gap-1 mobile:min-w-full mobile:flex-row justify-center items-center sm:hidden`}>
					{isApyLoading ? (
						<Skeleton w={50} h={30} />
					) : (
