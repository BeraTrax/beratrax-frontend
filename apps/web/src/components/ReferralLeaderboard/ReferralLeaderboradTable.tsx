			...
			<div className="text-textWhite flex flex-col gap-2">
				{isLoading ? (
					...
					allEntries.map((userTVL, index) => {
						...
						return <StatsTableRow key={userTVL.id} index={userTVL.referralRanking} isCurrentUser={isCurrentUser} {...userTVL} />;
					})
				) : (
					<EmptyTable />
				)}
...
const StatsTableRow = ({ isCurrentUser, ...props }: { isCurrentUser?: boolean } & Record<any, any>) => {
	...
	const { address, earnedTrax, earnedTraxByReferral, tvl, accountInfo, index, referralCount, referralCountWithNonZeroStake } = props;
