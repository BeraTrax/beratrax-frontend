				...
				<table className="w-full text-sm text-left">
					...
					<tbody className="font-league-spartan">
						{withdrawals?.map((withdrawal) => {
							...
							return (
								<tr key={withdrawal.id} className="border-b border-borderDark hover:bg-bgDark transition-colors duration-200">
									...
									<td className="px-6 py-4 text-textSecondary">
										{formatDistance(new Date(parseInt(withdrawal.blockTimestamp) * 1000), new Date(), {
											addSuffix: true,
										})}
									</td>
									<td className="px-6 py-4">
										<div className="flex items-center gap-2">
											...
											<FiExternalLink
												...
												onClick={() => window.open(`${BLOCK_EXPLORER_URL}/address/${withdrawal.from}`, "_blank")}
											/>
										</div>
									</td>
									...
									<td className="px-6 py-4 text-right text-textPrimary">${customCommify(shareValue)}</td>
									<td className="px-6 py-4 text-right text-textWhite">
