...
import { IClients } from "@beratrax/core/src/types";
import { getContract, encodeFunctionData, Address } from "viem";
import { awaitTransaction } from "@beratrax/core/src/utils/common";
import { pointsStakingAndClaimAbi } from "@beratrax/core/src/assets/abis/pointsStakingAndClaim";
import { additionalAirdropClaimAbi } from "@beratrax/core/src/assets/abis/additionalAirdropClaim";
...
export const fetchAirdropData = createAsyncThunk(
	...
	async ({ address, getClients }: { address: string; getClients: (chainId: number) => Promise<IClients> }, thunkApi) => {
		try {
...
export const withdrawAirdrop = createAsyncThunk(
	...
	async ({ amount, getClients }: { amount: bigint; getClients: (chainId: number) => Promise<IClients> }, thunkApi) => {
		try {
			const airdropAddress = addressesByChainId[CHAIN_ID.BERACHAIN].airdropAddress;
			const clients = await getClients(CHAIN_ID.BERACHAIN);
...
export const fetchAdditionalAirdropData = createAsyncThunk(
	...
	async ({ address, getClients }: { address: Address; getClients: (chainId: number) => Promise<IClients> }, thunkApi) => {
		try {
			...
			const [amount, pendingRewards] = await Promise.all([contract.read.stakes([address]), contract.read.pendingRewards([address])]);
...
export const claimAdditionalAirdrop = createAsyncThunk(
	...
	async ({ claim, nonce, getClients }: { nonce: number; claim: boolean; getClients: (chainId: number) => Promise<IClients> }, thunkApi) => {
		try {
			const state = thunkApi.getState() as { account: StateInterface };
...
export const { updateAccountField, reset, setLastVisitedPage, updateAirdropField, updateAdditionalAirdropField } = accountSlice.actions;
