...
const StatInfo = ({
	...
	return (
		<div className="flex items-center gap-4 bg-bgDark py-4 px-4 mt-2 rounded-2xl backdrop-blur-lg">
			{typeof iconUrl === "string" ? <img src={iconUrl} alt={title} className="flex-shrink-0 flex-grow-0 w-10 h-10" /> : iconUrl}
...
const PoolInfo = ({ farm, marketCap, vaultTvl, marketCapLoading, vaultTvlLoading }: IProps) => {
	...
	return (
		<div className=" mt-4 relative">
			{description && (
				<>
					<h3 className="text-textWhite font-arame-mono font-normal text-[16px] leading-[18px] tracking-widest">ABOUT</h3>
					{/* {farm.id === 8 && (
			...
			<div className="mt-4 flex flex-col gap-2">
				...
				{isAutoCompounded ? (
					<StatInfo title="BeraTrax auto-compounded APY" value={beraTraxApy + "%"} iconUrl={<GoRocket color="white" size={25} />} />
				) : null}

				{isAutoCompounded && beraTraxApyWithPoints ? (
					<StatInfo title="BeraTrax APY with Points" value={beraTraxApyWithPoints} iconUrl={<GoRocket color="white" size={25} />} />
				) : null}
				{farmApys.merklApr > 0 && (
					<StatInfo
