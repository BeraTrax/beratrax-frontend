...
import { ETFVault } from "@beratrax/core/src/types";
...
interface ETFVaultItemProps {
	vault: ETFVault;
}

const ETFVaultItem: FC<ETFVaultItemProps> = ({ vault }) => {
				...
				<View className="flex flex-row justify-between align-top gap-2">
					...
					<View className="flex-col gap-1">
						<View className="flex flex-row items-center gap-2 mb-2 justify-end">
							<FarmRowChip text={[vault?.originPlatform].filter(Boolean).join(" | ")} color="invert" />
				...
				<View className="flex flex-row justify-between gap-4">
					...
					<View className={`flex-1 basis-0 ml-4 ${estimateTrax ? "border-r border-r-bgPrimary" : ""}`}>
						...
						<Text className="text-textWhite text-lg font-league-spartan leading-5">
							{toFixedFloor(apy || 0, 2) == 0 ? "--" : `${customCommify(apy || 0, { minimumFractionDigits: 0 })}%`}{" "}
						</Text>
					</View>
