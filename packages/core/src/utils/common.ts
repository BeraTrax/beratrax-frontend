...
export const subtractGas = async (
	...
	const balance = _balance ? _balance : BigInt(store.getState().tokens.balances[client.public.chain.id][zeroAddress]?.valueWei || "0");
	...
	while (true) {
		if (txCount === 10) {
			...
			if (allowance >= parseUnits("10", 6)) yield true;
				...
				const hash = await contract.write.approve([addressesByChainId[CHAIN_ID.ARBITRUM].universalPaymaster!, maxUint256]);
...
export const checkPaymasterApproval = async (client?: IClients) => {
	if (!paymasterApprovalCheckIterator && client) paymasterApprovalCheckIterator = checkForPaymasterApprovalGenerator(client);
