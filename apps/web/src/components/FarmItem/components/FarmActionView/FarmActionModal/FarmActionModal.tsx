...
import { noExponents, toWei, toEth } from "@beratrax/core/src/utils/common";
...
import { ZappingDisclaimerModal } from "web/src/components/modals/ZappingDisclaimerModal/ZappingDisclaimerModal";
	...
	return (
		<MobileModalContainer open={open}>
			<div className="px-4 py-4">
				...
				<div className={`${FarmDetailsStyles.farmDetailsWrapper} flex flex-col items-center gap-4 text-textWhite   text-center`}>
					...
					<p className={`text-[18px] leading-[20px] break-words	 ${noOrMaxInputValue ? "text-textSecondary" : "text-textWhite"}`}>
						{!showInUsd ? "$" : ""}
										...
										<QuickDepositButtons
											...
											isSelected={
												text === "MAX"
													...
													: Math.abs(Number(amount) - Number((Number(maxBalance) * parseInt(text)) / 100)) < 0.0001
											}
										/>
									))
										...
										onClick={() => {
											if (text === "MAX") {
												...
												if (showInUsd) {
													...
													const earningsInTokens = currentVaultEarningsUsd / prices[farm.chainId][farm.vault_addr];
			...
			{confirmDeposit ? (
				<ConfirmFarmActionModal
					...
					depositInfo={{
						amount:
							transactionType === FarmTransactionType.Withdraw && Math.abs(Number(amount) - Number(currentEarnings)) < 0.0001
								? showInUsd
