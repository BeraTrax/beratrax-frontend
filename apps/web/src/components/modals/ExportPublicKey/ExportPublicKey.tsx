import { useApp, useWallet } from "@beratrax/core/src/hooks";
    ...
    >
      <div className="tablet:text-left">
        ...
        <div className="flex justify-center items-center gap-2 text-gray-500 mb-5">
          <p className="max-w-[377px] truncate">{`${currentWallet?.substring(0, 20)}...${currentWallet?.substring(
        ...
        <p className="w-[250px] hidden md:block">
          Sending cryptocurrency has never been easier. Simply scan this QR code to transfer your desired tokens to your
          Beratrax wallet.
        </p>
      </div>
      <div
        className={"bg-bgPrimary w-fit h-fit text-center p-3 m-auto border-[12px] border-bgDark rounded-xl shadow-xl"}
      >
        {currentWallet && (
          <QRCode
            value={currentWallet}
