...
export const getAllowanceSlot = (chainId: number, tokenAddress: Address, owner: Address, spender: Address) => {
	return keccak256(
		encodePacked(
			["uint256", "uint256"],
			[BigInt(spender), BigInt(keccak256(encodePacked(["uint256", "uint256"], [BigInt(owner), slots[chainId][tokenAddress].allowance])))]
		)
	);
};

export const getBalanceSlot = (chainId: number, tokenAddress: Address, holderAddress: Address) => {
	return keccak256(encodePacked(["uint256", "uint256"], [BigInt(holderAddress), slots[chainId][tokenAddress].balance]));
};
