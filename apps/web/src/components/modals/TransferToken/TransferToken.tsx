...
export const TransferToken: FC<IProps> = ({ token, handleClose }) => {
	...
	return (
		<ModalLayout onClose={handleClose} wrapperClassName="lg:w-full">
			<form
				className={"w-full tablet:w-auto flex flex-col justify-center items-center  font-league-spartan text-textWhite"}
			...
			>
				...
				<div className={"flex flex-col w-full my-2 "}>
					<label htmlFor="amount" className={"text-textSecondary text-lg"}>
						Amount: <span style={{ fontSize: 14 }}>(Balance: {showInUsd ? `$${token.usdBalance}` : token.balance})</span>
					</label>
					<div className={" flex justify-between items-center rounded-2xl gap-2 bg-gradientSecondary relative py-3 px-2"}>
						<input
							id="amount"
							type="number"
							autoComplete="off"
