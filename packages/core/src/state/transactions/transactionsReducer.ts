...
export const editTransactionDb = createAsyncThunk("transactions/editTransactionDb", async (transaction: EditTransaction, _thunkApi) => {
	try {
...
export const editTransactionStepDb = createAsyncThunk(
	...
	async (
		...
		try {
			const tx = (_thunkApi.getState() as RootState).transactions.transactions.find((item) => item._id === params.transactionId)!;
...
export const deleteTransactionDb = createAsyncThunk("transactions/deleteTransactionDb", async (transactionId: string, _thunkApi) => {
	try {
...
export const markAsFailedDb = createAsyncThunk("transactions/markAsFailedDb", async (transactionId: string, _thunkApi) => {
	try {
		if (!transactionId) throw new Error("transaction not found");
		const tx = (_thunkApi.getState() as RootState).transactions.transactions.find((item) => item._id === transactionId)!;
...
export const checkPendingTransactionsStatus = createAsyncThunk("transactions/checkPendingTransactionsStatus", async (_, thunkApi) => {
	try {
		...
		return thunkApi.rejectWithValue(error instanceof Error ? error.message : "Failed to check pending transactions status");
	}
});
