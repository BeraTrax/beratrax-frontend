...
export const PointsEarnings: React.FC<Props> = () => {
  ...
  const stakingPoints =
    ((totalEarnedTrax || 0) > (earnedTrax || 0)
      ? totalEarnedTrax
      : earnedTrax) ?? 0;
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      setClipPathValue(
        window.innerWidth < 768 ? mobileClipPath : desktopClipPath
      );
    };
  ...
  return (
    <View className={`flex flex-row gap-4 ${stakingPoints ? "gap-5" : ""}`}>
      <View className="flex-1 rounded-3xl border border-borderDark p-2 bg-[url('/src/assets/images/referralBg.png')] bg-cover bg-center bg-no-repeat">
        <Image
          source={ReferralLogo as ImageSourcePropType}
          alt="Referral Logo"
          className="mt-[-0.5rem] mb-[0.5rem] w-28 h-28"
        />
        <View className="pl-3">
          ...
          {boosts && boosts.includes(Boosts.BETA) && (
            <GradientText>x1.5 Multiplier - BETA 🚀</GradientText>
          )}
          {boosts && boosts.includes(Boosts.BETA_TESTER) && (
            <GradientText>x2 Multiplier - BETA TESTER 🚀</GradientText>
          )}
        </View>
      </View>
      ...
      >
        <Image
          source={DailyRateLogo as ImageSourcePropType}
          alt="Daily Rate Logo"
          className="ml-[-1.5rem] mt-[-1.5rem] w-32 h-32"
        />
        <Text
        ...
        <Text className="font-arame-mono font-normal text-bgDark text-sm pl-3">
