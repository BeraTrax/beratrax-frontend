...
export const TokenBalances: FC<IProps> = () => {
	...
	if (isLoading) {
		return (
			<div>
				<p className="font-arame-mono font-normal text-[16px] text-textWhite leading-4 uppercase mt-5">Token Balances</p>
	...
	return (
		<>
			...
			{(UIState === UIStateEnum.SHOW_TOKENS_LP || UIState === UIStateEnum.SHOW_TOKENS) && filteredLpTokens.length > 0 && (
				<>
					<p className="font-arame-mono font-normal text-[16px] text-textWhite leading-4 uppercase mt-5 ">Unstaked LP Token Balances</p>
					<div className={containerClass}>
						{filteredLpTokens.map((token, i) => (
							<div key={i} className={tokenCardStyle} onClick={() => setSelectedToken(token)}>
								<span className="flex">
									...
									{token.logo2 && <img className="w-9 h-9 -ml-3" src={token.logo2} alt="logo" style={{ clipPath: "circle(50%)" }} />}
								</span>
								<div>
									<p className="text-white text-lg">
										{token.name}
										{token.network ? <span className="ml-1 text-sm font-normal text-textPrimary ">({token.network})</span> : null}
									</p>
									<p className="text-textSecondary text-base">{token.balance}</p>
								</div>
...
const Settings = () => {
	...
	return (
		<OutsideClickHandler display="inline-block" onOutsideClick={() => setOpen(false)}>
			<div className="relative">
				<div className={" bg-bgPrimary w-5 h-5 rounded-3xl cursor-pointer p-[2px] pl-[2]"} onClick={() => setOpen(!open)}>
					<BiSliderAlt />
				</div>
				{open && (
					<div
