...
import FarmApyGraph from "@beratrax/web/src/pages/FarmInfo/FarmApyGraph/FarmApyGraph";
import { useState } from "react";
import FarmEarningsGraph from "web/src/pages/FarmInfo/FarmEarningsGraph/FarmEarningsGraph";
import FarmTvlGraph from "web/src/pages/FarmInfo/FarmTvlGraph/FarmTvlGraph";
				...
				<div className="flex justify-between">
					<div>
						...
						{isLpPriceLoading ? (
							...
							<div className="mt-2">
								<h1 className="text-textWhite text-5xl font-bold ">${customCommify(lp?.[0]?.lp || 0)}</h1>
					...
					<div className="flex flex-col mt-2 mr-3">
						<div className="flex items-center gap-2 mb-2 justify-end">
							<FarmRowChip text={[farm.originPlatform, farm.secondary_platform].filter(Boolean).join(" | ")} color="invert" />
							<div className="flex">
								<img alt={farm?.platform_alt} className="w-4 rounded-full border border-bgDark" src={`/${farm?.platform_logo}`} />
								{farm.secondary_platform && (
									<img className="w-4 rounded-full border border-bgDark" src={`/${farm?.secondary_platform_logo}`} />
								)}
							</div>
						</div>
						<div className="flex">
							{farm?.logo1 ? (
								<img alt={farm?.alt1} className={`mr-[-5px] h-20 md:mr-0 lg:h-20 max-w-fit rounded-full`} src={farm?.logo1} />
							) : null}

							{farm?.logo2 ? (
								<img alt={farm?.alt2} className={`w-20 h-20 lg:h-20 lg:w-20 ml-[-10px] max-w-fit rounded-full`} src={farm?.logo2} />
							) : null}

							{farm?.logo3 ? (
								<img alt={farm?.alt3} className={`w-20 h-20 lg:h-20 lg:w-20 ml-[-10px] max-w-fit rounded-full`} src={farm?.logo3} />
							) : null}
						</div>
					</div>
				</div>
