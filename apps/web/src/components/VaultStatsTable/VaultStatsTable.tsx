				...
				<table className="w-full min-w-[600px]">
					<thead>
						<tr className="text-textBlack text-left tracking-wide uppercase bg-bgPrimary border-b border-borderDark">
							...
							<th className="p-4 cursor-pointer hover:text-textSecondary" onClick={() => handleSort("platform")}>
								PLATFORM {sortColumn === "platform" && (sortDirection === "asc" ? "↑" : "↓")}
							</th>
							<th className="p-4 cursor-pointer hover:text-textSecondary" onClick={() => handleSort("depositedTvl")}>
							...
							sortedVaults
								...
								.map(
									({
										...
										<tr key={_id} className="border-b border-borderDark hover:bg-bgDark transition-colors">
											<td className="p-4">
												<div className="flex items-center gap-2 group relative">
													<p>
														{name} {isDeprecated ? <span className="text-red-500">Deprecated</span> : ""}
													</p>
											...
											<td className="p-4 ">{[originPlatform, secondaryPlatform].filter(Boolean).join(" | ")}</td>
											<td className="p-4 ">
