					...
					<tr className={styles.header}>
						...
						<th>
							<div className={styles.tableData + " " + styles.heading} onClick={() => setSortBy("tvl")}>
								TVL from Referrals
								{sortBy === "tvl" && (
									<FaArrowDown size={14} className={`${styles.transitionDelay} ${sortBy === "tvl" ? "" : styles.rotate}`} />
								)}
							</div>
						</th>
						<th>
							<div className={styles.tableData + " " + styles.heading} onClick={() => setSortBy("referred")}>
								Total Referred
								{sortBy === "referred" && (
									<FaArrowDown size={14} className={`${styles.transitionDelay} ${sortBy === "referred" ? "" : styles.rotate}`} />
								)}
							</div>
						</th>
					</tr>
					...
					{currentItems && currentItems.length > 0 ? (
						currentItems.map((referral, i) => (
							<tr key={uuid()} className={styles.tableRow}>
								...
								<td>
									{referral.address ? (
										<div className={styles.tableData + " " + styles.addressCol}>
											<p className={styles.addressText + " " + styles.onlyExtraLargeScreen}>{referral.address}</p>
											<p className={styles.addressText + " " + styles.onlyLargeScreen}>
												{`${`${referral.address}`.substring(0, 12)}...${`${referral.address}`.substring(
											...
											<p className={styles.addressText + " " + styles.onlyTablet}>
												{`${`${referral.address}`.substring(0, 8)}...${`${referral.address}`.substring(`${referral.address}`.length - 8)}`}{" "}
												<span className={styles.tooltip}>{`${referral.address}`}</span>
											</p>
											<p className={styles.addressText + " " + styles.onlyMobile}>
												{`${`${referral.address}`.substring(0, 5)}...${`${referral.address}`.substring(`${referral.address}`.length - 3)}`}{" "}
												<span className={styles.tooltip}>{`${referral.address}`}</span>
											</p>
											<FiExternalLink
												size={16}
												className={styles.arbiscanIcon}
												onClick={() => window.open(`${BLOCK_EXPLORER_URL}/address/${`${referral.address}`}`, "_blank")}
											/>
										</div>
									) : (
								...
								<td>
									<div className={`${styles.tableData}${" " + styles.specificCell}`}>{referral.referreredAddresses.length}</div>
								</td>
							</tr>
						))
					) : (
					...
					<tr className={styles.footer}>
						...
						<td colSpan={5}>
							<div className={styles.tableData + " " + styles.controls}>
								{currentPage > 1 && <FaArrowLeft className={styles.pageChangeArrow} onClick={() => changePage(currentPage - 1)} />}
								<p>
									Page {currentPage} of {totalPages}
								</p>
								{currentPage < totalPages && (
									<FaArrowRight className={styles.pageChangeArrow} onClick={() => changePage(currentPage + 1)} />
								)}
							</div>
						</td>
					</tr>
