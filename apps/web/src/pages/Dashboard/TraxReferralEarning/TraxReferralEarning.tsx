...
import ReferralLogo from "src/assets/images/referralLogo.png";
import DailyRateLogo from "src/assets/images/dailyRateLogo.png";
...
export const TraxReferralEarning: React.FC<Props> = () => {
    ...
    const { earnedTrax, totalEarnedTrax, totalEarnedTraxByReferral, earnedTraxByReferral, boosts } = useAppSelector(
        (state) => state.account
    );
    ...
    return (
        <div className={`flex gap-4 ${stakingPoints ? "gap-5" : ""}`}>
            <div className="flex-1 rounded-3xl border border-borderDark p-2 text-textWhite bg-[url('/src/assets/images/referralBg.png')] bg-cover bg-center bg-no-repeat">
                <img src={ReferralLogo} alt="Referral Logo" className="mt-[-0.5rem] mb-[0.5rem] w-28" />
                <div className="pl-3">
                    ...
                    {boosts && boosts.includes(Boosts.BETA) && (
                        <p
                            className="animate-pulse bg-gradient-to-r from-yellow-400 via-orange-500 to-teal-400 
                        bg-clip-text text-transparent font-extrabold text-lg
                        drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]
                        hover:scale-105 transition-transform duration-200"
                        >
                            x1.5 Multiplier - BETA 🚀
                        </p>
                    )}
                    {boosts && boosts.includes(Boosts.BETA_TESTER) && (
                        <p
                            className="animate-pulse bg-gradient-to-r from-yellow-400 via-orange-500 to-teal-400 
                        bg-clip-text text-transparent font-extrabold text-lg
                        drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]
                    ...
                    {boosts && boosts.includes(Boosts.LBGT_VAULT_MULTIPIER) && (
                        <p
                            className="animate-pulse bg-gradient-to-r from-yellow-400 via-orange-500 to-teal-400 
                        bg-clip-text text-transparent font-extrabold text-lg
                        drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]
            ...
            >
                <img src={DailyRateLogo} alt="Daily Rate Logo" className="ml-[-1.5rem] mt-[-1.5rem] w-32" />
                <p className={`font-arame-mono font-normal text-base text-bgDark leading-4 pl-3`}>
                    TOTAL POINTS FROM <br />
                    REFERRALS
                </p>
                <p className="font-league-spartan font-bold text-bgDark text-3xl leading-10 pl-3 pt-2">
                    {customCommify(referralPoints || 0, { minimumFractionDigits: 0, showDollarSign: false })}
                </p>
                <p className="font-arame-mono font-normal text-bgDark text-sm pl-3">
