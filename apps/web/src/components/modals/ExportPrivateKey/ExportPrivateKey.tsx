...
export const ExportPrivateKey: FC<IProps> = ({ setOpenModal }) => {
	...
	return (
		...
		>
			...
			<div>
				<input type="checkbox" name="confirm" id="confirm" onChange={() => setConfirm((prev) => !prev)} className="mr-2" />
			...
			<div className="bg-bgPrimary  w-full rounded-xl p-4 my-3 relative">
				{show ? (
					<>
						...
						/>
						{copied ? (
							<BsCheckCircle className="absolute right-[100px] top-1/2 transform -translate-y-1/2 cursor-pointer text-inherit" size={22} />
						) : (
							<BiCopy
