...
import { StatsGraph } from "src/components/Status/StatsGraph/StatsGraph";
...
export const Status = (): React.JSX.Element => {
    ...
    const runData = runs.map((run: AutoCompoundResult) => {
        ...
        const earnHasBlockchainErrors = vaults.some(
            (v: VaultData) => !v.earnSuccess && v.earnStatus?.toLowerCase().includes("reverted")
        );
        ...
        const earnBlockchainErrors = vaults.filter(
            (v: VaultData) => !v.earnSuccess && v.earnStatus?.toLowerCase().includes("reverted")
        ).length;
        ...
        const earnTooltip =
            earnBlockchainErrors > 0
                ? `Earn Succeeded: ${earnSucceeded}, Earn Failed: ${
                      totalVaults - earnSucceeded
                  } (${earnBlockchainErrors} blockchain errors)`
