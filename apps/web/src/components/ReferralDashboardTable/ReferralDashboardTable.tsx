            ...
            <th>
              <div className={styles.tableData + " " + styles.heading} onClick={() => setSortBy("referred")}>
                ...
                {sortBy === "referred" && (
                  <FaArrowDown
                    size={14}
                    className={`${styles.transitionDelay} ${sortBy === "referred" ? "" : styles.rotate}`}
                  />
                )}
              </div>
            </th>
                  ...
                  {referral.address ? (
                    <div className={styles.tableData + " " + styles.addressCol}>
                      ...
                      <p className={styles.addressText + " " + styles.onlyTablet}>
                        {`${`${referral.address}`.substring(0, 8)}...${`${referral.address}`.substring(
                          `${referral.address}`.length - 8
                        )}`}{" "}
                        <span className={styles.tooltip}>{`${referral.address}`}</span>
                      </p>
                      <p className={styles.addressText + " " + styles.onlyMobile}>
                        {`${`${referral.address}`.substring(0, 5)}...${`${referral.address}`.substring(
          ...
          <tr className={styles.footer}>
            ...
            <td colSpan={5}>
              <div className={styles.tableData + " " + styles.controls}>
                {currentPage > 1 && (
                  <FaArrowLeft className={styles.pageChangeArrow} onClick={() => changePage(currentPage - 1)} />
                )}
                <p>
