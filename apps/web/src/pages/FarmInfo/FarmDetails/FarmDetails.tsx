...
import { useSlippageDeposit, useSlippageWithdraw } from "@beratrax/core/src/hooks";
...
import { fetchAllPoolFeesThunk } from "@beratrax/core/src/state/fees/feesReducer";
...
const FarmDetailsRow: React.FC<{ farm: PoolDef; poolFees: PoolFees[]; isLoadingPoolFees: boolean }> = ({
	...
	const feesCollected = useMemo(() => {
		return poolFees.find((item) => item.pool_address.toLowerCase() === farm.vault_addr.toLowerCase())?.fees_usd || 0;
		...
		<>
			...
			>
				<div className={styles.slippageContainer}>
					...
					<div className={`dropdown ${lightMode && "dropdown--light"}`}>{!dropDown ? <RiArrowDownSLine /> : <RiArrowUpSLine />}</div>
				</div>
				{dropDown && (
					<div>
						<div className={"tvlMobileContainer"}>
							<div>
								<div style={{ width: "100%" }} className={`${styles.tvl_mobile} ${lightMode && styles["tvl_mobile--light"]}`}>
									TVL in pool
								</div>
								<div className={`${styles.tvl_underlying} ${lightMode && styles["tvl_underlying--light"]}`} style={{ width: "100%" }}>
									{totalSupplies[farm.chainId][farm.vault_addr].supply &&
										(Number(totalSupplies[farm.chainId][farm.vault_addr].supply) * lpPrice).toLocaleString("en-US", {
											style: "currency",
											currency: "USD",
				...
				{loadingDeposit ? (
					...
					<>
						{maxAmounts.map((maxAmount) =>
							tokens.map((token) => (
								<div key={`${maxAmount}-${token}`} className={`${styles.slippagecolor} ${lightMode && styles["slippagecolor--light"]}`}>
