...
export const UserStatsTable: FC = () => {
	const { userTVLs, page, setPage, hasNextPage, hasPrevPage, totalPages, sortBy, setSortBy, order, setOrder, search, setSearch } =
		useStats();
				...
				<table className="w-full min-w-[1000px]">
					<thead>
						<tr className="text-left tracking-wide uppercase bg-bgPrimary border-b border-borderDark">
							<th className="p-4 w-[140px]">
								<div className="flex items-center gap-2 cursor-pointer" onClick={() => handleSorting(UsersTableColumns.Address)}>
									ADDRESS
									{sortBy === UsersTableColumns.Address && (
										<FaArrowDown size={14} className={`transition-transform duration-200 ${order === "" ? "" : "rotate-180"}`} />
									)}
								</div>
							</th>
							<th className="p-4 w-[140px]">
							...
							<th className="p-4">
								<div className="flex items-center gap-2 cursor-pointer" onClick={() => handleSorting(UsersTableColumns.TVL)}>
									TVL
							...
							<th className="p-4">
								<div className="flex items-center gap-2 cursor-pointer" onClick={() => handleSorting(UsersTableColumns.TraxEarned)}>
									BTX points
							...
							<th className="p-4">
								<div className="flex items-center gap-2 cursor-pointer" onClick={() => handleSorting(UsersTableColumns.TraxEarnedRefferal)}>
									Ref. Points
							...
							<th className="p-4">
								<div className="flex items-center gap-2 cursor-pointer" onClick={() => handleSorting(UsersTableColumns.Multiplier)}>
									Multiplier
							...
							<th className="p-4">
								<div className="flex items-center gap-2 cursor-pointer" onClick={() => handleSorting(UsersTableColumns.ReferralCount)}>
									# of Referrals
					...
					<tbody className="text-textWhite">
						{userTVLs && userTVLs.length > 0 ? (
							userTVLs.map(({ id, address, tvl, accountInfo, earnedTrax, earnedTraxByReferral, referralCount, boosts }) => (
								<tr key={uuid()} className="border-b border-borderDark hover:bg-bgDark transition-colors">
									<td className="p-4">
										...
										{accountInfo && accountInfo.referrer ? (
											<div className="flex items-center gap-2 group relative">
												...
												<FiExternalLink
													size={16}
													className="text-textWhite cursor-pointer hover:text-textPrimary"
													onClick={() => window.open(`${BLOCK_EXPLORER_URL}/address/${accountInfo.referrer?.address}`, "_blank")}
												/>
											</div>
										) : (
											<div>-</div>
						...
						<tr className="border-t border-borderDark text-textWhite">
							...
							<td colSpan={5} className="p-4">
								<div className="flex items-center justify-end gap-4">
									{hasPrevPage && (
										<FaArrowLeft className="cursor-pointer hover:text-textPrimary" onClick={() => setPage((prev) => prev - 1)} />
									)}
									<p>
										Page {page} of {totalPages}
									</p>
									{hasNextPage && (
										<FaArrowRight className="cursor-pointer hover:text-textPrimary" onClick={() => setPage((prev) => prev + 1)} />
									)}
								</div>
							</td>
						</tr>
